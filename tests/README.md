...
### `claim-gate.test.ts` and `reader-agent.test.ts`
- Untrusted reader envelopes and candidates are validated independently
- Evidence must match exact document text at document-wide, end-exclusive offsets
- Claims cannot escape the segment that was sent to the reader
- Stable IDs are generated by the gate, never supplied by the reader
- One malformed claim or failed reader call does not destroy later valid results
